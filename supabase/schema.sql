...
-- İş İPTALİ (cancel_job) — taraflardan biri (ilan sahibi VEYA kabul edilmiş
-- teklifin sahibi/sürücü) eşleşmiş işi ATOMİK iptal eder: kabul edilen teklif
-- 'iptal', ilan yeniden 'aktif' (faz/sefer/kanıt sıfır) — iş panoya döner.
-- RPC şart: RLS altında sürücü kendi teklifini güncelleyemez (offers_update
-- yalnız ilan sahibine izinli) ve accepted_by_id'yi temizleyemez (driver guard);
-- istemci-tarafı iki-adımlı iptal yarım kalabilirdi. Detay: migration-2026-07-rpc-cancel-job.sql
...
select * from (values
  ('Ertuğrul İnşaat', true, 4.7::numeric, 'is','hafriyat','Dudullu şantiye hafriyat taşıma','İstanbul','Ümraniye','Dudullu OSB, blok C inşaatı','Samandıra döküm sahası','Hafriyat toprağı (kazı)',200::numeric,'ton','8-12 Haziran',true,'2 gün, günde ~5 sefer',null::text,null::text,'sabit',12500::numeric,'Bina kazısından çıkan hafriyat. Yükleme makinesi sahada mevcut. Taşıma mesafesi ~14 km.','aktif',0,'2 saat önce'),
  ...
  ('Başkent Altyapı İnşaat', true, 4.6, 'is','hafriyat','Yol genişletme — kazı fazlası taşıma','Ankara','Etimesgut','Eryaman yol çalışması','Belediye döküm alanı','Hafriyat toprağı (kazı)',150,'m³','10-15 Haziran',true,'2-3 gün',null,null,'sabit',9750,'Yol genişletmeden çıkan toprak. Birden fazla araca ihtiyaç var.','aktif',0,'3 saat önce'),
