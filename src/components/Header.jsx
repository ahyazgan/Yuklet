...
export default function Header({ darkMode, toggleDark, user, onLoginClick, onLogout, pendingOffersCount = 0, unreadCount = 0 }) {
  ...
  const navLink = "rounded-lg px-3 py-1.5 text-sm font-medium text-gray-600 dark:text-slate-300 transition hover:text-slate-950 dark:hover:text-slate-100";

  return (
    <header className="sticky top-0 z-50 border-b border-gray-100 dark:border-navy-line bg-white/90 dark:bg-navy-card/90 backdrop-blur-md">
      <div className="mx-auto flex h-[60px] max-w-6xl items-center justify-between px-4">
        <Link to="/" className="flex items-center gap-2.5">
          ...
          <div>
            <div className="text-lg font-extrabold tracking-tight text-slate-950 dark:text-slate-100">HamTed</div>
            <div className="text-[7.5px] font-semibold tracking-[3px] text-gray-400 dark:text-navy-muted">YÜK & NAKLİYE</div>
          </div>
        </Link>
        ...
        <nav className="hidden items-center gap-1 md:flex">
          <div className="relative" onMouseEnter={() => setShowCozumler(true)} onMouseLeave={() => setShowCozumler(false)}>
            ...
            {showCozumler && (
              <div className="absolute right-0 top-full w-64 overflow-hidden rounded-2xl border border-gray-100 dark:border-navy-line bg-white dark:bg-navy-card p-2 shadow-xl">
                {COZUMLER.map((c) => (
                  <Link key={c.to} to={c.to} onClick={() => setShowCozumler(false)} className="flex items-center gap-2.5 rounded-xl px-3 py-2.5 transition hover:bg-gray-50 dark:hover:bg-navy-soft">
                    <span className="text-xl">{c.icon}</span>
                    <div>
                      <div className="text-sm font-bold text-slate-950 dark:text-slate-100">{c.label}</div>
                      <div className="text-[11px] text-gray-400 dark:text-navy-muted">{c.sub}</div>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          {user ? (
            <>
              <Link to="/profil" title={user.email} className="rounded-lg px-3 py-1.5 text-sm font-bold text-slate-950 dark:text-slate-100">👤 {user.name}</Link>
              <button onClick={onLogout} aria-label="Çıkış yap" className="rounded-lg px-3 py-1.5 text-sm font-semibold text-gray-600 dark:text-slate-300 transition hover:text-slate-950 dark:hover:text-slate-100">Çıkış</button>
            </>
          ) : (
            <button onClick={onLoginClick} aria-label="Giriş yap" className="rounded-lg px-3 py-1.5 text-sm font-semibold text-gray-600 dark:text-slate-300 transition hover:text-slate-950 dark:hover:text-slate-100">Giriş yap</button>
          )}
      ...
      {mobileMenu && (
        <div className="flex flex-col gap-1.5 border-t border-gray-100 dark:border-navy-line bg-white dark:bg-navy-card px-4 py-3 md:hidden">
          {[...COZUMLER.map((c) => ({ label: `${c.icon} ${c.label}`, to: c.to })), ...NAV_ITEMS].map((item) => (
            <button key={item.to} onClick={() => handleNav(item.to)} className="rounded-xl bg-slate-50 dark:bg-navy-soft px-4 py-2.5 text-left text-sm font-medium text-slate-700 dark:text-slate-300">{item.label}</button>
          ))}
          {USER_ITEMS.map((item) => (
            <button key={item.label} onClick={() => handleNav(item.to)} className="rounded-xl bg-slate-50 dark:bg-navy-soft px-4 py-2.5 text-left text-sm font-medium text-slate-700 dark:text-slate-300">
              {item.label}{item.badge > 0 ? ` (${item.badge})` : ""}
            </button>
          ))}
          {user && <button onClick={() => handleNav("/profil")} className="rounded-xl bg-slate-50 dark:bg-navy-soft px-4 py-2.5 text-left text-sm font-medium text-slate-700 dark:text-slate-300">Profil</button>}
          {user ? (
            <button onClick={() => { onLogout(); setMobileMenu(false); }} className="rounded-xl bg-slate-50 dark:bg-navy-soft px-4 py-2.5 text-left text-sm font-medium text-slate-700 dark:text-slate-300">Çıkış ({user.name})</button>
          ) : (
            <button onClick={() => { onLoginClick(); setMobileMenu(false); }} className="rounded-xl bg-slate-50 dark:bg-navy-soft px-4 py-2.5 text-left text-sm font-medium text-slate-700 dark:text-slate-300">Giriş yap</button>
          )}
