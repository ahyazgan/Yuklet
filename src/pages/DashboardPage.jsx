...
function StatCard({ icon, label, value, sub, accent = "text-slate-950 dark:text-slate-100", chip = "bg-slate-100 dark:bg-navy-soft" }) {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3 rounded-3xl bg-white dark:bg-navy-card p-4 shadow-sm">
      <div className="flex items-center gap-2">
        <div className={`flex h-9 w-9 items-center justify-center rounded-full text-base ${chip}`}>{icon}</div>
        <span className="text-[11px] font-medium text-gray-500 dark:text-slate-400">{label}</span>
      </div>
      <div>
        <div className={`text-3xl font-extrabold tracking-tight ${accent}`}>{value ?? "—"}</div>
        {sub && <div className="mt-1 text-[11px] font-medium text-gray-400 dark:text-navy-muted">{sub}</div>}
      </div>
    </motion.div>
  );
}
...
const QUICK = "rounded-full bg-white dark:bg-navy-card px-4 py-2.5 text-xs font-semibold text-slate-700 dark:text-slate-100 shadow-sm transition hover:bg-gray-50 dark:hover:bg-navy-soft";
  ...
  if (!user) {
    return (
      <div className="mx-auto flex w-full max-w-[460px] flex-col items-center gap-3 px-4 pt-12 text-center text-slate-900 dark:text-slate-100">
        ...
        <h1 className="text-2xl font-extrabold tracking-tight text-slate-950 dark:text-slate-100">Paneli görmek için giriş yapın</h1>
  ...
  return (
    <div className="mx-auto flex w-full max-w-[460px] flex-col gap-5 px-4 pb-24 pt-2 text-slate-900 dark:text-slate-100">
      ...
      <div className="flex items-center justify-between gap-3 pt-2">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-black tracking-tight text-slate-950 dark:text-slate-100">Merhaba, {user.name} 👋</h1>
          </div>
      ...
      <h2 className="text-2xl font-bold leading-tight tracking-tight text-slate-950 dark:text-slate-100">Faaliyet özetin.</h2>
      ...
      <div className="rounded-3xl bg-white dark:bg-navy-card p-5 shadow-sm">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-base font-bold text-slate-950 dark:text-slate-100">{isNakliyeci ? "Araç ilanlarım" : "İlanlarım"}</h2>
          <button onClick={() => navigate("/ilanlarim")} className="text-xs font-semibold text-gray-500 dark:text-slate-400">Tümü →</button>
        </div>
        {myListings.length === 0 ? (
          <div className="py-5 text-center text-sm text-gray-400 dark:text-navy-muted">
            Henüz ilan yok.<br />
          ...
          <div className="flex flex-col gap-2.5">
            {myListings.slice(0, 5).map((l) => {
              ...
              return (
                <div key={l.id} onClick={() => navigate(`/ilan/${l.id}`)} className="flex cursor-pointer items-center gap-2.5 rounded-2xl border border-gray-100 dark:border-navy-line p-3">
                  <CategoryIcon catId={l.cat} size={18} fallback={cat?.icon} />
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-sm font-bold text-slate-950 dark:text-slate-100">{l.title}</div>
                    <div className="text-[11px] text-gray-500 dark:text-slate-400">{l.il} • {lOffers.length} teklif</div>
                  </div>
      ...
      {/* Son aktivite */}
      <div className="rounded-3xl bg-white dark:bg-navy-card p-5 shadow-sm">
        <h2 className="mb-3 text-base font-bold text-slate-950 dark:text-slate-100">Son aktivite</h2>
        {activity.length === 0 ? (
          <div className="py-5 text-center text-sm text-gray-400 dark:text-navy-muted">Henüz aktivite yok.<br />İlan vererek veya teklif göndererek başlayın.</div>
        ) : (
          <div className="flex flex-col gap-2.5">
            {activity.map((a) => (
              <div key={a.key} onClick={() => navigate(a.link)} className="flex cursor-pointer gap-2.5 rounded-2xl border border-gray-100 dark:border-navy-line p-3">
                <span className="flex-shrink-0 text-lg">{a.icon}</span>
                <div className="min-w-0 flex-1">
                  <div className="text-sm leading-snug text-slate-800 dark:text-slate-100">{a.text}</div>
                  {a.date && <div className="mt-1 text-[11px] text-gray-400 dark:text-navy-muted">{fmtDate(a.date)}</div>}
                </div>
              </div>
            ))}
          </div>
